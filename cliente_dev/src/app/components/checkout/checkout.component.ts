...
export class CheckoutComponent implements OnInit {
      ...
      response=>{
        ...
        this.carrito_arr.forEach(element => { 
            if(this.currency != 'PEN'){
              ...
              this.dventa.push({
                producto: element.producto._id,
                subtotal: Math.round(parseInt(element.producto.precio) - (parseInt(element.producto.precio)*this.descuento_activo.descuento)/100),
                variedad: element.variedad._id,
                cantidad: element.cantidad,
                cliente: localStorage.getItem('_id')
              });
  ...
  select_direccion_envio(item:any){
    ...
    //aqui establece el precio que se cobrara por el envio cajamarca y provincias
    }else if(this.direccion_envio.pais == 'Perú'){
      if(this.direccion_envio.region == 'Cajamarca'){
        if(this.direccion_envio.distrito == 'Jaén'){
          this.envio = 5;
        }else{
          this.envio = 10;
        }
        
      }else if(this.direccion_envio.region != 'Cajamarca'){
        this.envio = 15;
      }
    }
