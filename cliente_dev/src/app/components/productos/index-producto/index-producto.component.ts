...
export class IndexProductoComponent implements OnInit {
  ...
  public productos_categoria: Array<any> = [];
          ...
          this._guestService.listar_productos_publico('').subscribe(
            response=>{
              ...
              if (this.filtro_search) {
                ...
                this.productos = this.productos.filter(item => term.test(item.titulo));
              } else {
    ...
    if(cat == 'todos'){
      this._guestService.listar_productos_publico('').subscribe(
        response=>{
          ...
          this.productos = response.data;
          this.load_data = false;
          // console.log('imprimo que viene por parametro cat = todos')
          //  console.log(cat);
      ...
      this._guestService.listar_productos_categoria_publico(cat).subscribe(
        response=>{
          this.productos = response.data;
          this.productos = this.productos.filter(item=>item.categoria.toLowerCase()==cat);
          // console.log('imprimo que viene por parametro cat')
          // console.log(cat);
          // console.log(this.productos);
          this.load_data = false;
